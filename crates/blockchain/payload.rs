...
pub fn apply_plain_transaction(
    ...
    // Use the 1D sum (pre-refund gas_used) for capacity, matching geth's GasPool
    // behavior. While EIP-8037 computes block.gas_used as max(regular, state),
    // the builder must use sum(tx_gas_used) for capacity to ensure validators
    // can re-execute all transactions within the gas limit.
